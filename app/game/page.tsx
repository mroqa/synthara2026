...
type MobileTab = 'map' | 'world' | 'quests';
...
export default function GamePage() {
  ...
  const handleLocationClick = (loc: typeof LOCATIONS[0]) => {
    ...
    // On mobile, switch to world view after picking a location
    setMobileTab('world');
  };
  ...
  const WorldViewPanel = (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', bottom: '1rem', left: 'clamp(1rem, 3vw, 2rem)', right: 'clamp(1rem, 3vw, 2rem)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.2rem' }}>
            <span style={{ fontSize: '1.5rem' }}>{loc.icon}</span>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1rem, 3vw, 1.4rem)', color: '#fff', textShadow: '0 2px 8px rgba(0,0,0,0.8)' }}>
              {loc.name}
            </h2>
          </div>
      ...
      {/* Narrative area */}
      <div style={{ flex: 1, padding: 'clamp(1rem, 3vw, 1.5rem) clamp(1rem, 3vw, 2rem)', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        {/* Scene description */}
        <div className="glass-panel" style={{ padding: 'clamp(1rem, 2vw, 1.5rem)' }}>
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--color-void)', overflow: 'hidden' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
          ...
          <span style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(0.85rem, 2vw, 1.1rem)', letterSpacing: '0.15em', color: 'var(--color-text-primary)' }}>
            SYNTHARA
          </span>
        </div>
        <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'nowrap' }}>
          <a href="/game/memory" className="btn btn-ghost btn-sm" style={{ padding: '0.4rem 0.6rem', minHeight: '36px' }}>
            <span>🧠</span><span className="nav-label"> Memory</span>
          </a>
          <a href="/game/quests" className="btn btn-ghost btn-sm" style={{ padding: '0.4rem 0.6rem', minHeight: '36px' }}>
            <span>📜</span><span className="nav-label"> Quests</span>
          </a>
          <a href="/game/profile" className="btn btn-ghost btn-sm" style={{ padding: '0.4rem 0.6rem', minHeight: '36px' }}>
            <span>⚔</span><span className="nav-label"> {player?.name || 'Me'}</span>
          </a>
        </div>
      </nav>

      {/* ── Desktop: 3-column grid ────────────────────────────── */}
      <div className="game-desktop-grid" style={{ flex: 1, display: 'grid', gridTemplateColumns: '220px 1fr 280px', overflow: 'hidden' }}>
        {WorldMapPanel}
        {WorldViewPanel}
        {QuestPanel}
      </div>

      {/* ── Mobile: single active panel (tab-controlled) ──────── */}
      <div className="game-mobile-panels" style={{ flex: 1, overflow: 'hidden', display: 'none' }}>
        {mobileTab === 'map' && WorldMapPanel}
        {mobileTab === 'world' && WorldViewPanel}
        {mobileTab === 'quests' && QuestPanel}
      </div>
      ...
      <div className="mobile-tab-bar">
        <button
          id="tab-map"
          className={`mobile-tab-bar-btn ${mobileTab === 'map' ? 'active' : ''}`}
        ...
        <button
          id="tab-world"
          className={`mobile-tab-bar-btn ${mobileTab === 'world' ? 'active' : ''}`}
        ...
        <button
          id="tab-quests"
          className={`mobile-tab-bar-btn ${mobileTab === 'quests' ? 'active' : ''}`}
      ...
      {/* Inline style for responsive game layout */}
      <style>{`
        .nav-label { display: inline; }

        @media (max-width: 768px) {
          .game-desktop-grid { display: none !important; }
          .game-mobile-panels { display: block !important; }
        }
